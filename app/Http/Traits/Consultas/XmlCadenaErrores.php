...
trait XmlCadenaErrores {
   ...
   public function integraConsulta($cuenta, $digito, $carrera, $consulta_datos)
   {
      ...
      // Verificamos si existen reglas de negocio aplicables que ha sido rotas
      $reglasRotas = $this->reglasXml($items);
      if ($reglasRotas!=[]) {
   ...
   public function reglasXml($items)
   {
      ...
      $reglasNegocio = array();
      // La fecha de terminacion no puede ser anterior a la fecha de inicio de carrera
      if ($items["_12_fechaTerminacion"]<$items["_11_fechaInicio"]) {
         $reglasNegocio['MSJ46'] = 'p12 mayor a p11';
      }
      // La fecha de expedicion del titulo profesional no puede ser anterior
      // a la fecha de terinación en el nodo carrera_Attr
      if ($items['_21_fechaExpedicion']<$items['_12_fechaTerminacion']) {
         $reglasNegocio['MSJ49'] = 'p21 menor a p12';
      }
      // la fecha de expedicion del titulo profesional no puede ser anterior
      // a la fecha de terminación en el nodo antecedente_Attr
      if ($items['_21_fechaExpedicion']<$items['_37_fechaTerminacion'] ) {
         $reglasNegocio['MSJ50'] = 'p21 menor a p37';
      }
      // la fecha expedición no puede ser posterior al dia de hoy en el nodo expediciónote
      $hoy = carbon::now()->format('Y-m-d');
      if ($items['_21_fechaExpedicion']>$hoy) {
         $reglasNegocio['MSJ51'] = 'p21 mayor a hoy';
      }
   ...
   public function cadenaOriginal($nodos,$tipo)
   {
      ...
      $cadenaOriginal .= $nodos['Carrera']['fechaTerminacion'].'|';
      ...
      if ($nodos['Expedicion']['idModalidadTitulacion']==1) {
         $cadenaOriginal .= $nodos['Expedicion']['fechaExamenProfesional'].'|'; // opcional
         $cadenaOriginal .= '|'; // opcional
      } else {
         $cadenaOriginal .= '|'; // opcional
         $cadenaOriginal .= $nodos['Expedicion']['fechaExencionExamenProfesional'].'|'; // opcional
      }
      ...
      $cadenaOriginal .= $nodos['Expedicion']['cumplioServicioSocial'].'|';
   ...
   public function tituloXml($nodos)
   {
      ...
      $nodos = $this->borraOpcionales($nodos);
   ...
   public function borraOpcionales($nodos)
   {
      // Elimina los nodos opcionales del XML
      unset($nodos['Carrera']['numeroRvoe']);
      if ($nodos['Expedicion']['fechaExamenProfesional']=='----') {
         unset($nodos['Expedicion']['fechaExamenProfesional']);
      }
      if ($nodos['Expedicion']['fechaExencionExamenProfesional']=='----') {
         unset($nodos['Expedicion']['fechaExencionExamenProfesional']);
      }
      if ($nodos['Antecedente']['fechaInicio']=='----') {
         unset($nodos['Antecedente']['fechaInicio']);
      }
   ...
   public function integraNodosUnam($folio,$datos,$cargo)
   {
      ...
      switch ($cargo) {
         case 'Jtit':
            ...
            break;
         case 'Director':
            $nombre='GUILLERMO';$apellidoPat='CASTILLO';$apeMat='CORDOVA';
            $curp='CACG620808HDFSRL05';$idCarg='3';$cargo='RECTOR';$titulo='LIC';
            $certR = 'CertificadoResponsable'; $noCertR='00001000000409388297';
   ...
   public function integraNodosSep($folio,$datos,$sello1,$certR)
   {
      ...
      // Nodo responsable 1
      $nombre='GUILLERMO';$apellidoPat='CASTILLO';$apeMat='CORDOVA';
      $curp='CACG620808HDFSRL05';$idCarg='3';$cargo='RECTOR';$titulo='LIC'; // UIES180831HDFSEP03
      $noCertR='00001000000409388297';
      ...
      $componentes['Antecedente'] = $this->antecedente_Attr($inst,$idTipoE,$tipoE,
                                                            $idEntFed,$entFed,
                                                            $fechaI,$fechaT,$noCedula);
      // dd('integraNodosSep',$componentes);
      return $componentes;
   }
   public function loteCadena($fecha,$cargo)
   {
      ...
      foreach ($datos as $datosAlumno) {
         $responsable = $this->integraNodosUnam($folio.$datosAlumno->num_cta,unserialize($datosAlumno->datos),$cargo);
   ...
   public function tituloElectronico_Attr($folio)
   {
     ...
     $datos = array();
     $datos['xmlns'] = "https://www.siged.sep.gob.mx/titulos/";
   ...
   public function firmaResp_AttrUnam($nombre,$apellidoPat,$apeMat,$curp,$idCarg,$cargo,$titulo)
   {
     $data = array();
     $data['nombre'] = $nombre;
     $data['apellidoPat'] = $apellidoPat;
     $data['apeMat'] = $apeMat;
     $data['curp'] = $curp;
     $data['idCargo'] = $idCarg;
     $data['cargo'] = $cargo;
