...
class ListadosController extends Controller
{
    public function Pdfs()
    {
        ...
        // dd(count($data),$rpp,$limitesPDF);
        $vista = $this->listaHTML($data,$corte,$lista,$limitesPDF); // generacion del content del PDF

        // $view = \View::make('pdf.listaPDF', compact('data', 'corte', 'lista'))->render();
        // $view = \View::make('consultas.listasPDF', compact('data', 'corte', 'lista'))->render();
        $view = \View::make('consultas.listasPDF', compact('vista'))->render();
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($view);
        return $pdf->stream('listaPDF.pdf',array("Attachment"=>0));
    }
    ...
    public function listaHTML($data,$corte,$lista,$limitesPDF)
    {
        ...
        for ($i=0; $i < $paginas ; $i++)
        {
            ...
            $composite .= "<div id='logo'>";
            $composite .= "<img src='images/escudo_unam_solow.svg' alt=''>";
            $composite .= "</div>";
            $composite .= "<div id='invoice'>";
            $composite .=     "<h1>CORTE: ".$corte."</h1>";
            $composite .= "<div class='date'>Listado:".$lista."</div>";
            $composite .= "</div>";
            $composite .= "</div>";
            ...
            $composite .= "</table>";
            // $composite .= "</div>";
            $composite .= "<footer>Hoja ".($i+1)." de ".$paginas."</footer>";
    ...
    public function acordionHtml($data,$corte)
    {
      ...
      for ($i=0; $i < count($data) ; $i++) {
        ...
        $composite .=              "Lista ".$x_list.":    ".count($data[$i])." solicitudes.";
        $composite .=              "</a>";
        $composite .=            "</h4>";
        $composite .=         "</div>";
