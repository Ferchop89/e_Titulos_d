...
class LoginController extends Controller
{
  ...
  // }
    ...
    public function login()
    {
      ...
      // dd($info, $cuenta.$verif, empty($info));
         ...
         if($fecha_nac == $fechaCaptura)
         {
            if($this->studentExist($credentials['num_cta']))
            {
               ...
               if($valida['curp-validada'] == 0)
               {
                  ...
                  $this->createUserLogin($info[count($info)-1]->dat_ncta.$info[count($info)-1]->dat_dig_ver, $fechaCaptura, $nombres['apellido1'], $nombres['apellido2'], $nombres['nombre'], $info[count($info)-1]->dat_curp, null,$fechaNacDat);
               }
               else {
